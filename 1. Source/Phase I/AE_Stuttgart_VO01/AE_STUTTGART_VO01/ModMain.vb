Module ModMain
    ...
    Sub Main()
        ...
        Try
            ...
            sQuery = sQuery & " SELECT T0.ItemCode,T0.ItemName,T0.U_POSDesc,T1.U_AB_EASIGrp1,T0.U_AB_EASIDept, NULL [ProductType],T0.SalUnitMsr,T0.CodeBars,T0.validFor,"
            sQuery = sQuery & " CASE WHEN ISNULL(T0.U_POSNoService,'FALSE') = 'FALSE' THEN 0 ELSE 1 END [U_POSNoService],"
            sQuery = sQuery & " CASE WHEN ISNULL(T0.U_POSNoGst,'FALSE') = 'FALSE' THEN 0 ELSE 1 END [U_POSNoGst],"
            sQuery = sQuery & " CASE WHEN ISNULL(T0.U_POSNonDisc,'FALSE') = 'FALSE' THEN 0 ELSE 1 END [U_POSNonDisc],"
            ...
            sQuery = sQuery & " SET ItemName = T1.ItemName,FrgnName = T1.U_POSDesc,EASIGroup = T1.U_AB_EASIGrp1, "
            sQuery = sQuery & " EASIDept = T1.U_AB_EASIDept,SalesUnitMsr = T1.SalUnitMsr, Barcode = T1.CodeBars, "
            ...
            sQuery = sQuery & " INNER JOIN (SELECT T0.ItemCode,T0.ItemName,T0.U_POSDesc,T1.U_AB_EASIGrp1,T0.U_AB_EASIDept,T0.SalUnitMsr,T0.CodeBars,T0.ValidFor,"
            sQuery = sQuery & " 		    CASE WHEN ISNULL(T0.U_POSNoService,'FALSE') = 'FALSE' THEN 0 ELSE 1 END [U_POSNoService],"
            sQuery = sQuery & " 			CASE WHEN ISNULL(T0.U_POSNoGst,'FALSE') = 'FALSE' THEN 0 ELSE 1 END [U_POSNoGst],"
            sQuery = sQuery & " 			CASE WHEN ISNULL(T0.U_POSNonDisc,'FALSE') = 'FALSE' THEN 0 ELSE 1 END [U_POSNonDisc],"
            sQuery = sQuery & " 			CASE WHEN ISNULL(T0.U_POSZeroPrice,'FALSE') = 'FALSE' THEN 0 ELSE 1 END  [U_POSZeroPrice] "
            ...
            sQuery = sQuery & " WHERE ItemCode IN (SELECT SWW FROM OITM)"
            ...
            End If
            '************ITEM GROUP SYNC CODE ON 16/09/2015 STARTS**************************
            Console.WriteLine("Item Group Sync starts", sFuncName)
            sQuery = "INSERT INTO [" & p_oCompDef.p_sIntDBName & "].dbo.AB_ItemGroup(Code,Name,SAPSyncDate,SAPSyncDateTime)"
            ...
            sQuery = sQuery & " WHERE T0.ItmsGrpCod NOT IN (SELECT Code FROM  [" & p_oCompDef.p_sIntDBName & "].dbo.AB_ItemGroup) "

            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Item Group Sync Query Exec " & sQuery, sFuncName)
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Calling ExecuteSQLQuery_DT()", sFuncName)

            If ExecuteSQLQuery_DT(P_sConString, sQuery, sErrDesc) <> RTN_SUCCESS Then
                Console.WriteLine("Error while synchoronizing item group", sFuncName)
            Else
                Console.WriteLine("Item group synchronization completed Successfully", sFuncName)
            End If

            '**********UPDATE ITEM GROUP CODE******************
            Console.WriteLine("Update Item Group starts", sFuncName)
            sQuery = "UPDATE [" & p_oCompDef.p_sIntDBName & "].dbo.AB_ItemGroup "
            ...
            sQuery = sQuery & " FROM [" & p_oCompDef.p_sIntDBName & "].dbo.AB_ItemGroup T0"
            ...
            If ExecuteSQLQuery_DT(P_sConString, sQuery, sErrDesc) <> RTN_SUCCESS Then
                Console.WriteLine("Error while updating item group", sFuncName)
            Else
                Console.WriteLine("Item group update Successful", sFuncName)
            End If

            '************DEPARTMENT SYNC CODE ON 16/09/2015 STARTS**************************
            Console.WriteLine("Department Sync starts", sFuncName)
            sQuery = "INSERT INTO [" & p_oCompDef.p_sIntDBName & "].dbo.AB_Department(Code,Name,Active,SAPSyncDate,SAPSyncDateTime)"
            sQuery = sQuery & " SELECT Code,Name,U_AB_Active,DATEADD(DAY, DATEDIFF(day, 0, GETDATE()), 0) [SAPSyncDate],GETDATE() [SAPSyncDateTime] "
            sQuery = sQuery & " FROM [" & p_oCompDef.p_sDataBaseName & "].dbo.[@AB_EASIDEPT] T0 "
            sQuery = sQuery & " WHERE T0.Code NOT IN (SELECT Code FROM [" & p_oCompDef.p_sIntDBName & "].dbo.AB_Department) "

            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Department Sync Query Exec " & sQuery, sFuncName)
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Calling ExecuteSQLQuery_DT()", sFuncName)

            If ExecuteSQLQuery_DT(P_sConString, sQuery, sErrDesc) <> RTN_SUCCESS Then
                Console.WriteLine("Error while synchoronizing Department", sFuncName)
            Else
                Console.WriteLine("Department synchronization completed Successfully", sFuncName)
            End If

            '*********UPDATE DEPARTMENT*************
            Console.WriteLine("Update department stats", sFuncName)
            sQuery = "UPDATE [" & p_oCompDef.p_sIntDBName & "].dbo.AB_Department"
            sQuery = sQuery & " SET Code = T1.Code,Name = T1.Name, SAPSyncDate = DATEADD(DAY,DATEDIFF(DAY,0,GETDATE()),0),SAPSyncDateTime = GETDATE()"
            sQuery = sQuery & " FROM [" & p_oCompDef.p_sIntDBName & "].dbo.AB_Department T0 "
            sQuery = sQuery & " INNER JOIN (SELECT T0.Code,T0.Name"
            sQuery = sQuery & " 	        FROM [" & p_oCompDef.p_sDataBaseName & "].dbo.[@AB_EASIDEPT] T0"
            sQuery = sQuery & " 	        WHERE T0.UpdateDate >= DATEADD(DAY,DATEDIFF(DAY,0,GETDATE()),- " & p_oCompDef.p_iIntegDays & ")"
            sQuery = sQuery & "            ) T1 ON T1.Code = T0.Code"

            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Department update Query Exec " & sQuery, sFuncName)
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Calling ExecuteSQLQuery_DT()", sFuncName)

            If ExecuteSQLQuery_DT(P_sConString, sQuery, sErrDesc) <> RTN_SUCCESS Then
                Console.WriteLine("Error while updating Department", sFuncName)
            Else
                Console.WriteLine("Department update Successful", sFuncName)
            End If
